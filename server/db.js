...
const db = new Database(path.join(dataDir, 'yuwamart.db'));
...
CREATE TABLE IF NOT EXISTS products (
  ...
  type TEXT NOT NULL DEFAULT 'grocery',   -- 'grocery' or 'fastfood'
  category TEXT,
  name TEXT NOT NULL,
  weight TEXT,
  price REAL NOT NULL,
...
if (countRow.c === 0) {
  const insert = db.prepare(`INSERT INTO products (type, category, name, weight, price, old_price, image_url)
                              VALUES (@type, @category, @name, @weight, @price, @old_price, @image_url)`);
  const groceries = [
  ...
  const insertMany = db.transaction((rows) => {
    for (const [category, name, weight, price, old_price] of rows) {
      insert.run({ type: 'grocery', category, name, weight, price, old_price, image_url: null });
    }
  });
  insertMany(groceries);
  ...
  const insertFF = db.transaction((rows) => {
    for (const [name, weight, price, old_price] of rows) {
      insert.run({ type: 'fastfood', category: 'fastfood', name, weight, price, old_price, image_url: null });
    }
  });
  insertFF(fastfood);
}
