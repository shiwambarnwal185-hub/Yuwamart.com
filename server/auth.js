...
router.post('/signup', async (req, res) => {
  try {
    ...
    const isValidPhone = /^9\d{9}$/.test(contact);
    const isValidEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact);
    if (!isValidPhone && !isValidEmail) {
      return res.status(400).json({ error: 'Please provide a valid 10-digit Nepali mobile number (starting with 9) or a valid email address.' });
    }
